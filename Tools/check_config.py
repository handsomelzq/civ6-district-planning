...
def main(argv):
    ...
    for r in rows_of("adjacency_rules.csv"):
        ...
        # 规则 21：**游戏自带的提示文本**必须点明产出来源。
        #   只审带 `{1_num}` 占位符的行 —— 那是游戏本地化字符串的标记，由
        #   parse_civ6_xml.py 原样搬进「备注」。「备注」本身是自由文本列
        #   （字段说明未给它任何格式约定），手写的注解不带占位符，不该被这条审。
        #   真实表 103 行全部带占位符，其中 102 行都有「相邻X」/「来自X」/
        #   「如靠近X」，唯一的例外是韩国书院那条 `自身` 规则（"+{1_Num}点 科技值。"）。
        #   这个缺失本身就是判断 `自身` 语义的先验证据（SDD §10 D10），两个方向都守：
        #     · 非「自身」却不点明来源 → 可能是又一条 Self 式规则，或解析漏了字段
        #     · 「自身」反倒点明了来源 → 补丁改了文案，那是支持读法 B 的直接证据，
        #       D10 可能因此能靠数据定案，必须有人看见
        #   一律给警告不给错误：本地化文案随补丁改措辞，不该让交付失败。
        note = r.get("备注", "")
        if re.search(r"\{1_[Nn]um", note):
            has_src = bool(re.search(r"相邻|来自|靠近|邻近", note))
            if cat != "自身" and not has_src:
                warn(f"adjacency {i} 的提示文本没点明产出来源：{note!r}"
                     f"（全库仅书院那条「自身」规则如此，见 SDD §10 D10；"
                     f"若这是又一条 Self 式规则，其语义需游戏内实测确认）")
            elif cat == "自身" and has_src:
                warn(f"adjacency {i} 是「自身」类别，但提示文本点明了来源：{note!r}"
                     f"（这是支持 SDD §10 D10「读法 B」的直接证据——"
                     f"请复核 D10 的结论与 SELF_IS_FIXED_VALUE）")
        # 规则 4：所需数量 >= 1
        n = r.get("所需数量", "").strip()
        if not is_int(n):
