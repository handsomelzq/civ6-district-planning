...
BROKEN_EXPECT = [
    ...
    ("建筑·替换不得指向自己", "的「替换建筑id」指向自己"),
    ("建筑·替换外键",        "的「替换建筑id」引用了不存在的 建筑"),
    ("建筑·宗教标记布尔",    "的「是否宗教建筑」应为 是/否"),
    ("地块·坐标重复",        "坐标重复"),
    ...
    ("警告·关卡无地块",      "没有任何地块"),
    # 规则 21（SDD §10 D10 的证据护栏）：只审带 {1_num} 占位符的游戏文案，两个方向都审。
    ("警告·提示文本无来源",  "的提示文本没点明产出来源"),
    ("警告·自身却有来源",    "是「自身」类别，但提示文本点明了来源"),
    # 规则 22：唯一性上限（districts.每城上限 / 每玩家上限）
    ("上限·须正整数或无限",  "「每城上限」应为正整数或「无限」"),
    ("上限·两列互相矛盾",    "为无限但「每玩家上限」为 1，两列互相矛盾"),
    ("上限·每玩家不得更小",  "的「每玩家上限」1 小于「每城上限」2"),
]
