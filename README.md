...
- **🔴 全部 9 关都是非法局面**（2026-09-26 发现）：补上 `districts.每城上限` / `每玩家上限` 两列后，求值器的唯一性校验立刻查出 9 关全部违规——文明 6 的一座城市每种专业化区域只能有一座，而 L-09 摆了 5 座书院、L-06/07/08 各摆了 2 座市政广场（全文明仅一座）。**这些局面在游戏里摆不出来。** 成因、范围与三条修法见 [设计/SDD-局面求值器.md](设计/SDD-局面求值器.md) §10 D12 与 [设计/关卡设计.md](设计/关卡设计.md) §9 D6。当前 E17b 已生效，9 关的违规集合冻结成特征测试
- **相邻上界仍是「忽略全局唯一性」的上界**：[设计/数值设计.md](设计/数值设计.md) §1 的 12–18 没有考虑同类区域不可重复，待关卡修法定下后重算
